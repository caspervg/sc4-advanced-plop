...
void BuildingsPanelTab::OnRender() {
    ...
    const float buildingsTableHeight = availHeight * 0.6f;
    const float lotsTableHeight = availHeight * 0.4f - ImGui::GetTextLineHeightWithSpacing();

    RenderBuildingsTable_(buildingsTableHeight);
...
void BuildingsPanelTab::OnDeviceReset(const uint32_t deviceGeneration) {
...
ImGuiTexture BuildingsPanelTab::LoadBuildingTexture_(const uint32_t buildingInstanceId) {
    ...
    // Find the building
    const auto& buildings = director_->GetBuildings();
    const auto it = std::ranges::find_if(buildings, [buildingInstanceId](const Building& b) {
        return b.instanceId.value() == buildingInstanceId;
    });

    if (it == buildings.end() || !it->thumbnail.has_value()) {
        return texture;
    }
    ...
    if (ImGui::BeginTable("BuildingsTable", 4, tableFlags, ImVec2(0, tableHeight))) {
        ...
        while (clipper.Step()) {
            ...
            for (int i = prefetchStart; i < prefetchEnd; ++i) {
                const uint32_t key = filteredBuildings_[i]->instanceId.value();
                thumbnailCache_.RequestLoad(key);
            }
        ...
        thumbnailCache_.ProcessLoadQueue([this](uint32_t key) {
...
void BuildingsPanelTab::RenderBuildingRow_(const Building& building, const bool isSelected) {
    const uint32_t key = building.instanceId.value();
